...
### Fixed
- Google batch submissions no longer drop the first URL: request keys now use `req-{index}` so index `0` is not treated as falsy by `Google\Http\Batch::add()` (`false == "0"`)
